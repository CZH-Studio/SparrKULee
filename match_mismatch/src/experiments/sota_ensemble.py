...
from match_mismatch.src.experiments.sota import LitSOTAModule


class LitSOTAEnsembleModule(LitBaseModule):
    def __init__(self, cfg: DictConfig):
        super().__init__(cfg)
        self.model = nn.ModuleList()
        for model_path_ in cfg.model.sota_ensemble.using_models:
            model_path = Path(model_path_)
            if model_path.exists():
                cfg_new = rename_model_name(cfg, "sota")
                module = LitSOTAModule.load_from_checkpoint(model_path, cfg=cfg_new)
...
def main(cfg: DictConfig):
    ...
    cfg.model.model_name = "sota_ensemble"
    ...
    run(LitSOTAEnsembleModule, cfg, results_dir, immediate_save=True)
