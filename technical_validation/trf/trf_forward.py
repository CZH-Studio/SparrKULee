...
def predict_trf(
    model: Ridge, envelope: NDArray, eeg: NDArray, lag_min: int, lag_max: int
):
    """
    在测试数据上预测
    """
...
def main():
    ...
    for subject_dir in eeg_dir.glob("sub-*"):
        ...
        for i in range(len(subject_data_pairs)):
            ...
            corrs = predict_trf(
                model, test_envelope_data, test_eeg_data, lags[0], lags[-1]
            )
            cross_validation_results.append(np.mean(corrs))
        subject_result = np.mean(cross_validation_results)
        print(f"Mean prediction correlation of subject {subject}: {subject_result:.6f}")
        subject_results.loc[subject] = [subject_result]
    with open("trf_forward_corr.csv", "w", encoding="utf-8") as f:
        subject_results.sort_index().to_csv(f, index=True, index_label="subject")
