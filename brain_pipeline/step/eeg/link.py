...
class LinkEEG2Stimuli(Step):
    ...
    def __call__(self, input_data: Dict[str, Any], logger: Logger) -> Dict[str, Any]:
        ...
        # 加载刺激数据
        stimuli_loader = LoadStimuli(
            input_keys=["tmp_in"], output_keys=["tmp_data", "tmp_sr"]
        )
        ...
        if trigger_file == "n/a":
            ...
            stimuli_trigger_path = (
                base_dir / "stimuli" / tsv_events_data["trigger_file"]
            )
        stimuli_trigger_loader = LoadStimuliTrigger(
            input_keys=["tmp_in"], output_keys=["tmp_data", "tmp_sr"]
        )
        stimuli_trigger_dict = stimuli_trigger_loader(
            {"tmp_in": stimuli_trigger_path}, logger
        )
