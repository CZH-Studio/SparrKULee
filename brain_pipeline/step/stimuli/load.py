...
def fn_npz(output_keys: List[str], path: Path):
    data = np.load(path)
    return dict(zip(output_keys, [data["audio"].astype(np.float32), data["fs"].item()]))
...
class LoadStimuli(Step):
    def __init__(self, input_keys: OptionalKey = None, output_keys: OptionalKey = None):
        """Load stimuli from glob dataloader, supported stimuli format: `.wav, .mp3, .npz, .gz`

        :param input_keys: Key of stimuli file path, defaults to `[I_STI_PATH]`
        :type input_keys: OptionalKey
        :param output_keys: Data and sr of stimuli, defaults to `[I_STI_DATA, I_STI_SR]`
    ...
    def __call__(
        self, input_data: Dict[str, Any], logger: Logger
        ...
        logger.info(
            f"Loaded stimuli from {path.name} with shape {data[self.output_keys[0]].shape}"
        )
        return data
...
class LoadStimuliTrigger(Step):
    def __init__(self, input_keys: OptionalKey = None, output_keys: OptionalKey = None):
        """Load stimuli trigger file

        :param input_keys: Key of stimuli trigger file path, defaults to `[I_STI_TRIGGER_PATH]`
        :type input_keys: OptionalKey
        :param output_keys: Data and sr of stimuli trigger, defaults to `[I_STI_TRIGGER_DATA, I_STI_TRIGGER_SR]`
        :type output_keys: OptionalKey
        """
        super().__init__(
            input_keys,
    ...
    def __call__(
        ...
        data = fn(self.output_keys, path)
        logger.info(
            f"Loaded stimuli trigger from {path.name} with shape {data[self.output_keys[0]].shape}"
        )
        return data
